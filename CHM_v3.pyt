...
class Agl(object):
	...
	def getParameterInfo(self):
		pdata = [
			...
			['Run in Memory?', 'run_in_memory', 'GPBoolean', 'Optional', 'Input', 'Advanced Options']
		]
	...
	def execute(self, parameters, messages):
		...
		if mem:
			...
			diff_arr = self.subtract_array(dsm_arr, dtm_arr)
			...
			# ...or use Minus for great stability
			arcpy.SetProgressor('default', 'Doing Raster Math...')
			diff_calc = Minus(dsm, dtm)
